...
//	}
	...
	// else if (prob == 1) {
		// res = pow(2.0, (double) bin_set_size(vars_set));
	// }
	else {
...
vector <char> mutation(vector <char> v) {
	#pragma omp parallel for
	for (int i = 0; i < guessing_vars_cnt; ++i) {
		if (rand_int() % guessing_vars_cnt == 0)
			v[i] ^= 1;
	}
	return v;
}
...
void observe(vector <char> &start_point, ostream &os) {
	...
	os << "current point:\n";
	os << "complexity: " << current_complexity << "\n";
	os << "probability: " << current_probability << "\n";
	os << setw(5) << bin_set_size(current_set) << ":";
	for (int i = 0; i < guessing_vars_cnt; ++i) {
		if (current_set[i]) {
			os << setw(5) << guessing_vars[i];
		}
	}
	os << "\npoints has been viewed: " << points_cnt << " (" << time(0) - start_time << " sec)\n" << endl;
	
	int ssz = bin_set_size(current_set);
	/// ИЗМЕНЕНИЕ ОДНОЙ ПЕРЕМЕННОЙ
	for (int u = 0; u < guessing_vars_cnt; ++u) {
		current_set[u] = current_set[u] ^ 1;
		if (bin_set_size(current_set) > ssz) {
			current_set[u] = current_set[u] ^ 1;
			continue;
		}
