...
void solve(char &a, vector<char> &vars_values, vector<char> &is_def,
		vector<char> &answers, const vector <char> &save)
{
	map<pair <int, int>, int> gate_pairs;
	...
	for (int i = 0; i < (int) all_vars.size(); ++i) {
		...
		auto _vars_values = vars_values;
		auto _is_def = is_def;

		auto _gate_pairs = gate_pairs;
		auto _dsu = dsu;
		auto _classes = classes;
		auto _useless_equations = useless_equations;
		auto _linear_constraints = linear_constraints;
		auto _learnts = learnts;

		define_variable_value(var, val, _vars_values, _is_def, _dsu);

		while (true) {
			char cnt;

			while (true) {
				cnt = analyze_equations(_vars_values, _is_def, _dsu, _classes,
					_gate_pairs, _learnts, _useless_equations);

				vector<vector<int>> relations;
				simple_linear_propagation(_linear_constraints, relations, _vars_values, _is_def, _dsu);
				cnt |= analyze_relations(relations, _vars_values, _is_def, _dsu, _classes);

				if (!cnt)
					break;
			}

			cnt |= analyze_learnts(_learnts, _vars_values, _is_def, _dsu, _classes);

			if (!cnt) {
				vector<vector<int>> relations;
				auto linear_constraints_copy = _linear_constraints;
				linear_propagation(linear_constraints_copy, relations);
				cnt |= analyze_relations(relations, _vars_values, _is_def, _dsu, _classes);
			}
...
double complexity(const vector<char> &vars_set)
{
	...
	int cnt = 0;
	vector<int> cnts(all_vars.size(), 0);
	#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < N; ++i) {
		char answer;
		vector<char> vars_values(2 * (vars_cnt + 1));
		...
		vector<char> answers(all_vars.size(), 0);
		solve(answer, vars_values, is_def, answers, interp[i]);
		
		#pragma omp critical (cnts)
		{
			// clog << i << endl;
	...
	map<int, vector<int>> cnts_to_vars;
	for (int i = 0; i < (int) cnts.size(); ++i)
		cnts_to_vars[cnts[i]].push_back(all_vars[i]);

	for (auto &p: cnts_to_vars) {
		// double prob = (double) p.first / N;
		clog << p.first << " / " << N << ":\n";
		for (auto x: p.second)
			clog << x << " ";
		clog << "\n";
